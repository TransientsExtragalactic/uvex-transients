...
from uvex_transients.models._typing import (
    FloatArray,
    PhysicalInput,
    RNGInput,
    ScalarPhysicalValue,
)
from uvex_transients.models._utils import ensure_numpy_array
...
class Parameter:
    ...
    :class:`~uvex_transients.models.core.priors.Prior` to sample from, a
    ...
    :class:`~uvex_transients.models.core.priors.NormalPrior` via ``transform="log"``,
    ...
    # -------------------------------------- #
    _TRANSFORMS: ClassVar[dict[str, tuple[Callable[..., FloatArray], Callable[..., FloatArray]]]] = {
        "log": (np.log, np.exp),
    ...
    # -------------------------------------- #
    prior: Prior
    """~uvex_transients.models.core.priors.Prior: The prior distribution for the latent variable :math:`z`."""
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.prior, Prior):
            raise TypeError(
                "Parameter 'prior' must be an instance of uvex_transients.models.core.priors.Prior, "
                f"but got {type(self.prior).__name__}.",
            )

        if isinstance(self.scale, bool) or not isinstance(self.scale, (Quantity, float, int)):
            raise TypeError(
    ...
    def _resolve_transform(self) -> None:
        ...
        if self.transform is None:
            ...
            try:
                self.transform, self.inverse_transform = self._TRANSFORMS[self.transform]
            except KeyError:
                raise ValueError(
    ...
    def transform_physical_to_latent(self, x: PhysicalInput) -> FloatArray:
        ...
        # `scale` directly.
        x_arr: Quantity | FloatArray = x if isinstance(x, Quantity) else np.asarray(x, dtype=np.float64)
    ...
    def set_prior(self, prior: Prior) -> None:
        ...
        Parameters
        ----------
        prior : uvex_transients.models.core.priors.Prior
    ...
    def _check_fixed_value(self, value: ScalarPhysicalValue) -> None:
        ...
        with np.errstate(invalid="ignore", divide="ignore"):
            try:
                ...
                raise TypeError(
                    f"Fixed value {value!r} has units incompatible with this parameter's scale ({self.scale!r})."
                ) from exc

        if not np.isfinite(latent_value):
            raise ValueError(f"Fixed value {value!r} maps to a non-finite latent value ({latent_value}).")
