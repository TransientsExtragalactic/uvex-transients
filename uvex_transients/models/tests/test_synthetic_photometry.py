...
def test_synthetic_photometry_with_dust_extinction():
    ...
    PARAMETERS = {name: value[:, np.newaxis] for name, value in SED_MODEL.sample_parameters(n_events, rng=0).items()}
    ...
    spectra = SED_MODEL.as_source_spectrum(TIME_SINCE_EXPLOSION, redshift=REDSHIFT, **PARAMETERS)
...
def test_synthetic_photometry_with_batched_dust_extinction():
    ...
    chokes on a per-event batch axis), `uvex_transients.dust.log_attenuation`
