"""
Shared, inheritable pytest "contracts" for :mod:`uvex_transients.models`.

Rather than writing bespoke tests for every :class:`~uvex_transients.models.core.Lightcurve`/
:class:`~uvex_transients.models.core.Spectrum`/:class:`~uvex_transients.models.core.SpectralModel`
...
consistency invariants described in :mod:`uvex_transients.models.core._base` plus
...
def assert_full_coverage(base: type, tested: set[type], *, exclude: AbstractSet[type] = frozenset()) -> None:
    """
...
def _integration_bounds(model_class, cgs_params: dict) -> tuple[float, float]:
    ...
    :attr:`~uvex_transients.models.core._base.Spectrum._DOMAIN` (possibly infinite)
    otherwise.
    """
...
def _integrate_over_frequency(
    ...
    :func:`scipy.integrate.quad` handles fine once reparameterized to log
    ...
    integral, _ = quad(integrand, log_lo, log_hi, points=log_breakpoints or None, limit=400)
    return integral
...
class ModelContract:
    ...
    :class:`~uvex_transients.models.core._base.Lightcurve`,
    :class:`~uvex_transients.models.core._base.Spectrum`, or
    :class:`~uvex_transients.models.core._base.SpectralModel` subclass.
    """
...
class LightcurveContract(ModelContract):
    """Generic checks for :class:`~uvex_transients.models.core._base.Lightcurve` subclasses."""
...
class SpectrumContract(ModelContract):
    """Generic checks for :class:`~uvex_transients.models.core._base.Spectrum` subclasses."""
...
class SpectralModelContract(ModelContract):
    """
    Generic checks for :class:`~uvex_transients.models.core._base.SpectralModel` subclasses.

    Covers both plain ``SpectralModel`` subclasses and
    :class:`~uvex_transients.models.core._base.ComposedSpectralModel` subclasses --
    ...
    def test_eval_from_arrays_matches_eval(self):
        ...
        packed = self.model_class.pack_params_to_arrays(**params)
        np.testing.assert_allclose(
            self.model_class.eval_from_arrays(self.nu_grid, self.t_grid[0], *packed).cgs.value,
    ...
    def test_eval_spectrum_integrates_to_one(self):
        ...
        integral = _integrate_over_frequency_at_time(self.model_class, cgs_params, lo, hi, t0, "eval_spectrum_cgs")
    ...
    def test_eval_bolometric_matches_frequency_integral(self):
        ...
        expected = _integrate_over_frequency_at_time(self.model_class, cgs_params, lo, hi, t0, "eval_cgs")
    ...
    def test_as_astropy_model_matches_eval(self):
        ...
        model_nu = self.model_class.as_astropy_model(x_type="nu", y_type="nu", freq_unit=u.Hz, **cgs_params)
    ...
    def test_as_astropy_model_rejects_inconsistent_units(self):
        ...
        with pytest.raises(ValueError):
            self.model_class.as_astropy_model(x_type="lambda", wave_unit=u.Hz, **cgs_params)
        with pytest.raises(ValueError):
    ...
    def test_flux_matches_eval_at_zero_redshift(self):
        ...
        expected = self.model_class.eval(self.nu_grid, t0, **params) / (4 * np.pi * self.luminosity_distance**2)
    ...
    def test_as_astropy_model_matches_flux(self):
        ...
        expected = self.model_class.flux_cgs(nu0, t0, redshift_cgs, luminosity_distance_cgs, **cgs_params)
    ...
    def test_simulate_shape_and_finiteness(self):
        instance = self.model_class()
        result = instance.simulate(self.nu_grid[0], self.t_grid[0], size=5, rng=self.seed)
        assert result.shape == (5,)
        assert np.all(np.isfinite(result.cgs.value))
