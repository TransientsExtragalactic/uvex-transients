...
per-column type/unit/dtype checks (:class:`~uvex_transients.surveys.utils.QTableColumnSpec`),
plus per-action required-column and custom checks
(:class:`~uvex_transients.surveys.utils.ActionSpec`). All validation errors are collected and
...
class SurveySchedule:
    ...
    _ACTION_SCHEMA: dict[str, ActionSpec] = {
        "observe": ActionSpec(
            ...
            validator=lambda rows: True if np.all(rows["duration"] > 0) else "observation durations must be positive",
    ...
    def __init__(self, schedule_table: QTable, instrument_fov: SkyRegion, **kwargs):
        ...
        **kwargs
        ...
        if not isinstance(schedule_table, QTable):
            raise TypeError(f"Parameter 'schedule_table' must be of type QTable, not {type(schedule_table)}")
        ...
        self._max_action_duration = self._schedule_table["duration"].max() if len(self._schedule_table) else 0 * u.s
        ...
        self._start_time_jd = np.asarray(self._schedule_table["start_time"].jd, dtype=np.float64)
    ...
    def _validate_table_columns(self, **_) -> list[str]:
        ...
        for column_name, column_spec in self._SCHEMA.items():
            errors.extend(column_spec.validate_against_table(self._schedule_table, column_name))
    ...
    def __add__(self, other: "SurveySchedule") -> "SurveySchedule":
        ...
        if self.fov != other.fov:
            raise ValueError("Cannot combine schedules with different instrument fields of view.")
    ...
    def end_time(self) -> Time:
        ...
        end_times = self._schedule_table["start_time"] + self._schedule_table["duration"]
        ...
        return QTable(
            {
                ...
                "total_duration": u.Quantity([self.time_spent(name) for name in unique_actions]),
            }
        )
    ...
    def get_healpix_coverage_index(
        ...
        if len(observe_rows) == 0:
            ...
            row_indices = np.repeat(np.arange(len(pixel_arrays), dtype=np.int64), pixel_counts)
    ...
    def get_observations_of(
        ...
        rows = self.get_rows_between_times(start_time, end_time) if start_time is not None else self._schedule_table
    ...
    def compute_pair_count_curve(
        ...
        with logging_redirect_tqdm(loggers=[logger]):
            for timescale in tqdm(timescales, desc="Computing pair-count curve", unit="timescale"):
                self._validate_timescale(timescale)
    ...
    def compute_control_time_curve(
        ...
        with logging_redirect_tqdm(loggers=[logger]):
            progress = tqdm(timescales, desc="Computing control-time curve", unit="timescale")
