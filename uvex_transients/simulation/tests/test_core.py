...
def _make_schedule(n_sched=40, seed=0):
...
def _make_catalog(transient, n_events=30, seed=1):
    rng = np.random.default_rng(seed)
    ...
    coord = SkyCoord(
        np.where(in_footprint, _HOT_SPOT.ra.value, rng.uniform(0, 360, n_events)) * u.deg,
        np.where(in_footprint, _HOT_SPOT.dec.value, rng.uniform(-89, 89, n_events)) * u.deg,
    )
...
def test_filter_by_snr_matches_independent_unbatched_computation():
    """
    ...
    *not* the exact `get_observations_of`; see `filter_by_snr`'s own docstring for
    ...
    catalog, coord, t_explosion, redshift = _make_catalog(transient)
...
def test_filter_by_snr_discards_never_observed_events():
    """Events far outside the schedule's footprint are dropped without error."""
    transient = TidalDisruptionEvent()
    schedule = _make_schedule(n_sched=5)
...
def test_filter_by_snr_empty_catalog():
    transient = TidalDisruptionEvent()
    schedule = _make_schedule(n_sched=5)
    catalog, *_ = _make_catalog(transient, n_events=0)
...
def test_filter_by_snr_unknown_transient_type_raises():
    transient = TidalDisruptionEvent()
    schedule = _make_schedule(n_sched=5)
    catalog, *_ = _make_catalog(transient, n_events=1)
...
def test_filter_by_snr_unknown_band_raises():
    transient = TidalDisruptionEvent()
    schedule = _make_schedule(n_sched=5)
    catalog, *_ = _make_catalog(transient, n_events=1)
