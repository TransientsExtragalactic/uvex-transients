...
.. list-table::
   ...
   * - :class:`~uvex_transients.transients.supernovae.MagnetarSLSNe`
     ...
     - 600 d
     - 4
   * - :class:`~uvex_transients.transients.supernovae.TypeIaSNe`
     - :class:`~uvex_transients.models.supernovae.Ia.TypeIaSED`
     - 365 d
     - 1
...
``cosmology``-dependent. Leaving it at its default of `None` (as every built-in population still
without a sourced rate uncertainty does) means no rate uncertainty has been sourced yet -- every
bound below then collapses to the point estimate, twice over, rather than silently reading as "the
rate is known exactly."
